...
<head>
    ...
    <title>AKKUŞLAR SEYAHAT | TERMİNAL V104</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; transition: 0.3s; font-family: 'DM Sans', sans-serif; }
        ...
        /* YAYIN BUTONU */
        .live-btn { background: linear-gradient(90deg, #ff0050, #00f2ea); color: #000; font-weight: bold; padding: 18px; border-radius: 12px; margin-bottom: 25px; display: none; text-align: center; text-decoration: none; animation: pulse 1.5s infinite; border: 2px solid white; font-size: 16px; }
...
<div class="sidebar">
    ...
    <button class="nav-btn" onclick="sayfa(13, this)">⚙️ Yönetim Paneli</button>
...
<div class="main">
    ...
    <div id="p4" class="panel">
        ...
        <h1>📜 Akkuşlar Seyahat Kuralları</h1>
        <div class="card" style="text-align: left; line-height: 1.8;">
            <p>✅ 1. Kurucuya ve Yönetime saygı esastır.</p>
            <p>✅ 2. Konvoy sırasında gereksiz sollama ve kaza yapmak yasaktır.</p>
            <p>✅ 3. Telsiz ve sohbet üzerinden küfür/hakaret kesinlikle yasaktır.</p>
            <p>✅ 4. Ekip içinde yardımlaşma önceliğimizdir.</p>
            <p>✅ 5. Konvoylara mazeretsiz katılmamak disiplin suçudur.</p>
        </div>
    </div>

    <div id="p5" class="panel">
        <button class="back-btn" onclick="sayfa(1)">🔙 Geri</button>
        <h1>✍️ Ekibe Katılma Formu</h1>
        <div class="card">
            <p>Aramıza katılmak için adını soyadını yaz ve başvuruyu gönder!</p>
            <input type="text" id="k-ad" placeholder="Adınız ve Soyadınız">
            <button onclick="basvur()" class="action-btn">BAŞVURUYU GÖNDER</button>
        </div>
    </div>
    ...
    <div id="p13" class="panel">
        ...
        <div class="grid">
            <div class="card editor-only" style="border: 2px solid var(--admin);">
                <h3>🎥 YAYIN & ROTA AYARI</h3>
                <input type="text" id="inp-guzergah" placeholder="Güzergah Yaz">
                <button class="action-btn" style="background:var(--gold); color:black;" onclick="setGuzergah()">ROTAYI GÜNCELLE</button>
                <hr style="border:1px solid #333; margin:15px 0;">
                <input type="text" id="inp-kaptan" placeholder="Kaptan İsmi">
                ...
                <button class="action-btn" onclick="setLive()">YAYIN BİLGİSİNİ GÜNCELLE</button>
            </div>
    ...
    <div id="p15" class="panel">
        ...
        <div class="grid">
            <div class="card" style="border: 2px solid #00f2ea;">
                <h3>Kelebek Misali (TikTok)</h3>
                <a href="https://www.tiktok.com/@kelebekmiisaliii" target="_blank" class="action-btn" style="background:#00f2ea; color:black;">TIKLA VE GİT</a>
            </div>
            <div class="card" style="border: 2px solid #ff0050;">
                <h3>Akkuş Ailesi (TikTok)</h3>
                <a href="https://www.tiktok.com/@akkusailesi20" target="_blank" class="action-btn" style="background:#ff0050; color:white;">TIKLA VE GİT</a>
            </div>
        </div>
    </div>

    <div id="p2" class="panel"><button class="back-btn" onclick="sayfa(1)">🔙 Geri</button><h1>👥 Ekip Üyeleri</h1><div id="list-uye-adlari" class="grid"></div></div>
    <div id="p3" class="panel"><button class="back-btn" onclick="sayfa(1)">🔙 Geri</button><h1>🛡️ Yönetim Kadrosu</h1><div id="list-yon-adlari" class="grid"></div></div>
    <div id="p8" class="panel"><button class="back-btn" onclick="sayfa(1)">🔙 Geri</button><h1>🖼️ Galeri</h1><div id="list-galeri" class="grid"></div></div>
    <div id="p7" class="panel"><button class="back-btn" onclick="sayfa(1)">🔙 Geri</button><h1>🛠️ Modlar & Müzikler</h1><div id="list-mod" class="grid"></div></div>
    <div id="p21" class="panel"><button class="back-btn" onclick="sayfa(1)">🔙 Geri</button><h1>🎫 Sanal Bilet Al</h1><div class="card"><input type="text" id="b-ad" placeholder="Ad Soyad"><input type="text" id="b-nereye" placeholder="Varış"><button class="action-btn" onclick="biletAl()">BİLET AL</button><div id="list-bilet" class="grid" style="margin-top:20px;"></div></div></div>
    <div id="p12" class="panel"><button class="back-btn" onclick="sayfa(1)">🔙 Geri</button><h1>🛡️ Yönetim Telsizi</h1><div class="card"><div id="msg-list-yon" style="height:300px; overflow-y:auto; background:#000; padding:15px; border-radius:10px; border:2px solid var(--gold); text-align:left;"></div><input type="text" id="nick-yon" placeholder="İsim"><input type="text" id="msg-yon" placeholder="Mesaj..."><button onclick="mesajGonder('sohbet_yon', 'nick-yon', 'msg-yon')" class="action-btn">GÖNDER</button></div></div>
</div>
...
<script>
    ...
    function sayfa(n, btn) {
        if(n === 12 && aktifRol === "uye") { let s = prompt("Telsiz Şifresi:"); if(s !== "akkuslar7/24") return; }
    ...
    // YAYIN VE GÜZERGAH SİSTEMİ
    function setGuzergah() { 
        const v = document.getElementById('inp-guzergah').value; 
        db.collection("terminal").doc("guncel").set({guzergah: v}, {merge:true}).then(() => alert("Rota Güncellendi!")); 
    }
    function setLive() { 
        ...
        db.collection("terminal").doc("guncel").set({durum: s, link: l, kaptan: k}, {merge:true}).then(() => alert("Yayın Güncellendi!")); 
    }
    ...
    db.collection("terminal").doc("guncel").onSnapshot(doc => {
        if(doc.exists()) {
            ...
            if(d.durum == "1") {
                banner.innerText = `🔴 KAPTAN ${d.kaptan || "BİRİ"} ŞU AN YAYINDA! TIKLA KATIL!`;
                banner.href = d.link || "#";
    ...
    // BAŞVURU SİSTEMİ
    function basvur() { 
        const ad = document.getElementById('k-ad').value;
        if(ad) { db.collection("basvurular").add({ad: ad, tarih: Date.now()}); alert("Başvurunuz Alındı!"); document.getElementById('k-ad').value = ""; }
    }
    ...
    db.collection("basvurular").onSnapshot(s => {
        if(aktifRol!=='uye') document.getElementById('list-basvuru').innerHTML = s.docs.map(d => `<div class="card">${d.data().ad} <button onclick="basvuruyuOnayla('${d.id}','${d.data().ad}')">ONAYLA</button></div>`).join('');
    });
    ...
    function sil(c, id) { if(aktifRol==='kurucu' && confirm("Sileyim mi kaptan?")) db.collection(c).doc(id).delete(); }

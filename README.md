...
<head>
    ...
    <title>AKKUŞLAR SEYAHAT | TERMİNAL V86</title>
    ...
    <style>
        :root { --p: #CC0000; --bg: #0A0A0A; --card: rgba(20,20,20,0.95); --gold: #FFD700; --admin: #00CCFF; --yon: #AA00FF; }
        * { box-sizing: border-box; transition: 0.3s; font-family: 'DM Sans', sans-serif; }
        ...
        .main { flex: 1; padding: 40px; overflow-y: auto; background: rgba(0,0,0,0.6); z-index: 5; }
        .panel { display: none; }
        ...
        /* RESİM ÇERÇEVESİ HATASIZ */
        .img-fix { width: 100%; border-radius: 8px; height: 180px; object-fit: cover; border: 1px solid #444; margin-bottom: 10px; }
    </style>
</head>
<body id="app-body">
...
<div class="main">
    <div id="p1" class="panel aktif">
        <div class="grid">
            <div class="card"><h3>Üye Sayısı</h3><h1 id="count-uye" style="color:var(--p)">0</h1></div>
            <div class="card"><h3>Yönetim</h3><h1 id="count-yon" style="color:var(--gold)">0</h1></div>
        </div>
        <div class="card">
            <h3>📢 Ekip Telsizi</h3>
            <div id="msg-list-uye" style="height:250px; overflow-y:auto; background:rgba(0,0,0,0.8); padding:15px; border-radius:10px;"></div>
    ...
    <div id="p13" class="panel">
        ...
        <div class="grid">
            ...
            <div class="card kurucu-only">
                ...
                <button class="action-btn" onclick="setBg()">ARKA PLANI AYARLA</button>
            </div>
...
<script>
    ...
    function setBg() { 
        ...
        if(url) { db.collection("ayarlar").doc("tema").set({bg: url}); alert("Arka plan mühürlendi!"); }
    }
    ...
    db.collection("icerik").orderBy("tarih","desc").onSnapshot(s => {
        ...
        s.docs.forEach(d => {
            const dt = d.data();
            // BURASI HATALIYDI, DÜZELTİLDİ:
            const h = `<div class="card"><img src="${dt.url}" class="img-fix"><br><b>${dt.bas}</b><br><button class="del-btn" onclick="sil('icerik','${d.id}')">SİL</button></div>`;
    ...
    function sil(c, id) { if(confirm("Kaptan, silinsin mi?")) db.collection(c).doc(id).delete(); }
</script>
</body>
</html>

...
<head>
    ...
    <title>AKKUŞLAR SEYAHAT | TERMİNAL V45</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; transition: 0.3s; }
        ...
        .hero-img { width: 100%; height: 450px; object-fit: cover; border-radius: 20px; border: 4px solid var(--p); margin-bottom: 20px; }
        ...
        /* SİLME BUTONU (SADECE KURUCUDA) */
        .del-btn { background: #600; color: white; border: none; padding: 5px 10px; border-radius: 5px; cursor: pointer; display: none; margin-top: 10px; }
        ...
        canvas { background: #111; border: 2px solid var(--p); display: block; margin: 0 auto; border-radius: 10px; }
    </style>
</head>
<body id="app-body">
...
<div class="sidebar">
    ...
    <button class="nav-btn" onclick="sayfa(3, this)">🎫 Bilet Kes</button>
    <button class="nav-btn" onclick="sayfa(10, this)">🎮 Minyatür Oyun</button>
    <button class="nav-btn" onclick="sayfa(5, this)">🎨 Kaplama & Mod</button>
...
<div class="main">
    <div id="p1" class="panel aktif">
        <img id="main-afis" class="hero-img" style="display:none;">
        <h1 style="font-size: 40px; color: var(--p); margin-bottom: 0;">AKKUŞLAR SEYAHAT</h1>
        <p style="color: var(--gold); font-weight: bold;">Hükümdar: 乂✯ҠƐꝈƐβƐҠ✯乂</p>
        <div class="grid">
            <div class="card"><h3 id="count-uye">0</h3><p>Kayıtlı Üye</p></div>
            <div class="card"><h3>İST-DEN-ANK-HAT</h3><p>Aktif Hatlar</p></div>
        </div>
    </div>

    <div id="p2" class="panel"><h1>👥 Ekip Listesi</h1><div id="list-ekip" class="grid"></div></div>
    ...
    <div id="p3" class="panel">
        ...
        <div class="grid">
            <div class="card">
                ...
                <select id="b-rota"><option>Bartın-Denizli</option><option>İstanbul-Hatay</option></select>
                <button class="action-btn" onclick="biletKes()">BİLETİ BAS</button>
            </div>
            <div class="card"><h3>Yolcu Listesi</h3><div id="list-bilet" style="text-align:left;"></div></div>
        </div>
    </div>

    <div id="p5" class="panel"><h1>🎨 Kaplamalar & Modlar</h1><div id="list-icerik" class="grid"></div></div>

    <div id="p10" class="panel"><h1>🎮 Akkuşlar Tır Sürüşü</h1><canvas id="gameCanvas" width="600" height="400"></canvas></div>

    <div id="p12" class="panel">
        <h1>📩 Ekibe Katıl</h1>
        <div class="card" style="max-width:500px; margin:auto;">
            <input type="text" id="k-ad" placeholder="Nickiniz...">
            <input type="text" id="k-tk" placeholder="TikTok Adınız...">
            <button class="action-btn" onclick="basvuruYap()">BAŞVURUYU GÖNDER</button>
        </div>
        <div id="onay-kutusu" style="display:none; margin-top:20px;">
            <h3>📥 Bekleyen Başvurular (Admin/Kurucu)</h3>
    ...
    <div id="p13" class="panel">
        ...
        <div class="grid">
            <div id="panel-kurucu" class="card" style="display:none; border:2px solid var(--gold);">
                ...
                <button class="action-btn" onclick="rolVer()">ROLÜ MÜHÜRLE</button>
                <hr>
                <input type="text" id="afis-link" placeholder="Afiş Resim URL...">
                <button class="action-btn" onclick="afisSet()" style="background:var(--gold); color:#000;">AFİŞİ DEĞİŞTİR</button>
            </div>

            <div id="panel-editor" class="card" style="display:none; border:2px solid #00CCFF;">
...
<script>
    ...
    function yonetimGiris() {
        ...
        if(s === "1907akkus") {
            ...
            document.getElementById('onay-kutusu').style.display = "block";
            sayfa(13);
        } else if(s === "admin123") {
            rol = "admin";
            document.getElementById('panel-editor').style.display = "block";
    ...
    db.collection("basvuru").onSnapshot(s => {
        document.getElementById('list-basvuru').innerHTML = s.docs.map(d => `<div class="card"><b>${d.data().ad}</b><br><button onclick="onay('${d.id}','${d.data().ad}')" style="background:green; color:white; border:none; padding:5px; border-radius:5px; cursor:pointer;">ONAYLA</button> <button class="del-btn" onclick="sil('basvuru','${d.id}')" style="display:inline-block !important;">REDDET</button></div>`).join('');
    });
    ...
    function afisSet() { if(rol === "kurucu") db.collection("ayarlar").doc("afis").set({url: document.getElementById('afis-link').value}); }
    function icerikEkle() { if(rol !== "uye") db.collection("icerik").add({bas: document.getElementById('i-bas').value, url: document.getElementById('i-url').value}); }
    function basvuruYap() { db.collection("basvuru").add({ad: document.getElementById('k-ad').value, tk: document.getElementById('k-tk').value}); alert("İletildi!"); }
    function biletKes() { db.collection("biletler").add({ad: document.getElementById('b-ad').value, rota: document.getElementById('b-rota').value}); }
    function onay(id, ad) { db.collection("ekip").add({ad: ad, rol: 'uye'}); db.collection("basvuru").doc(id).delete(); }
    ...
    function initGame() {
        ...
        function loop() {
            ...
            road += 5; ctx.fillStyle="#CC0000"; ctx.fillRect(x, 300, 50, 80); ctx.fillStyle="#000"; ctx.fillRect(x+5, 310, 40, 20);
            ctx.fillStyle="#FFF"; ctx.font="9px Arial"; ctx.fillText("AKKUŞLAR", x+2, 350); requestAnimationFrame(loop);
        }

...
<head>
    ...
    <title>AKKUŞLAR SEYAHAT | TERMİNAL V53</title>
...
<div class="main">
    <div id="p1" class="panel aktif">
        ...
        <div class="grid" style="margin-top:20px;">
            <div class="card"><h3 id="top-uye-sayi">0</h3><p>Toplam Kaptan</p></div>
            <div class="card"><h3 id="top-admin-sayi">0</h3><p>Admin & Yönetici</p></div>
        </div>
    </div>
    ...
    <div id="p12" class="panel"><h1>📩 Katılım</h1><div class="card" style="max-width:500px; margin:auto;"><input type="text" id="k-ad" placeholder="Adınız..."><button class="action-btn" onclick="basvuruAt()">GÖNDER</button></div><div id="onay-kutu" style="display:none; margin-top:20px;"><div id="list-basvuru" class="grid"></div></div></div>
    ...
    <div id="p13" class="panel">
        ...
        <div class="grid">
            <div id="p-kurucu" class="card" style="display:none; border:2px solid var(--gold);">
                ...
                <hr>
                <input type="file" id="afis-file"><button class="action-btn" onclick="dosyaYukle('afis')">AFİŞ YÜKLE</button>
            </div>
            <div id="p-editor" class="card" style="display:none;"><input type="text" id="i-bas" placeholder="Başlık..."><input type="file" id="i-file"><button class="action-btn" onclick="dosyaYukle('icerik')">PAYLAŞ</button></div>
        </div>
    </div>
</div>
...
<script>
    ...
    function yonetimGiris() {
        let s = prompt("Şifre:");
        if(s === "1907akkus") { rol="kurucu"; document.getElementById('app-body').classList.add('kurucu-modu'); document.getElementById('p-kurucu').style.display="block"; document.getElementById('p-editor').style.display="block"; document.getElementById('onay-kutu').style.display="block"; sayfa(13); }
        else if(s === "admin123") { rol="admin"; document.getElementById('p-editor').style.display="block"; document.getElementById('onay-kutu').style.display="block"; sayfa(13); }
    }
    ...
    // ÜYE KADROSU TAKİBİ (İKONLU VE AYRILMIŞ)
    db.collection("ekip").onSnapshot(s => {
        ...
        s.docs.forEach(d => {
            ...
            let badgeClass = data.rol === "admin" ? "badge-admin" : (data.rol === "yonetici" ? "badge-yonetici" : "badge-uye");
            ...
            kadroHtml += `
                <div class="card">
                    <span class="badge ${badgeClass}">${ikon} ${data.rol}</span><br>
                    <b style="font-size:18px;">${data.ad}</b><br>
                    <button class="del-btn" onclick="sil('ekip','${d.id}')">KADRODAN SİL</button>
                </div>`;
        });
        document.getElementById('kadro-listesi').innerHTML = kadroHtml;
        document.getElementById('top-uye-sayi').innerText = uSayi;
        document.getElementById('top-admin-sayi').innerText = aSayi;
    });
    ...
    function basvuruAt() { db.collection("basvuru").add({ad: document.getElementById('k-ad').value, tarih: Date.now()}); alert("Gönderildi!"); }
    ...
    function initGame() {
        ...
        function loop() {
            ...
            road+=5; ctx.fillStyle="red"; ctx.fillRect(x, 300, 50, 80); ctx.fillStyle="#FFF"; ctx.font="9px Arial"; ctx.fillText("AKKUŞLAR", x+2, 350);
